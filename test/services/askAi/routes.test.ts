...
import { askAiService, SUGGESTION_FALLBACK_MESSAGE } from '../../../src/services/askAi/service';
...
import { createAiStreamRouter } from '../../../src/services/askAi/routes';
import { ForbiddenError } from 'apollo-server-express';
import type { AiStreamPart } from '@hawk.so/types';

jest.mock('../../../src/services/askAi/service', () => ({
  ...jest.requireActual('../../../src/services/askAi/service'),
  askAiService: {
...
describe('AI stream routes - GET /integration/ai/stream', () => {
  ...
  it('should return 403 when the user has no access to the project workspace', async () => {
    mockCheckUserInWorkspaceByProjectId.mockRejectedValue(new ForbiddenError('You have no access to this workspace'));
    const app = setupApp();
  ...
  it('should pass an unexpected workspace lookup failure to the outer error handler instead of returning 403', async () => {
    mockCheckUserInWorkspaceByProjectId.mockRejectedValue(new Error('connection to MongoDB lost'));
    const app = setupApp();
    ...
    expect((nextError as Error).message).toBe('connection to MongoDB lost');
  });
  ...
  it('should send an error part when the underlying stream throws', async () => {
    mockStreamSuggestion.mockResolvedValue((async function * () {
      yield {
        type: 'text-delta',
        delta: 'The stack trace ',

...
import PromoCodeService, {
  ...
  PromoCodeError,
  PromoCodeErrorCode
} from '../../src/utils/promoCodeService';
...
describe('PromoCodeService', () => {
  it('normalizes promo code value before lookup', () => {
  ...
  it('calculates amount discount with min final price', () => {
    const plan = createPlan({ monthlyCharge: 1000 });
    const price = calculatePromoCodePlanPrice({
  ...
  it('calculates fixed price promo', () => {
    const plan = createPlan({ monthlyCharge: 1000 });
    const price = calculatePromoCodePlanPrice({
      type: 'fixed_price',
      amount: 299,
  ...
  it('does not apply discount to plan outside applicablePlanIds', () => {
    const plan = createPlan({ monthlyCharge: 1000 });
    const price = calculatePromoCodePlanPrice({
      type: 'percent_discount',
      percent: 50,
  ...
  it('rejects total usage limit', async () => {
    const promoCode = createPromoCode({
      type: 'fixed_price',
      amount: 100,
    }, {
  ...
  it('rejects user usage limit', async () => {
    const promoCode = createPromoCode({
      type: 'fixed_price',
      amount: 100,
    });
  ...
  it('rejects workspace usage limit', async () => {
    const promoCode = createPromoCode({
      type: 'fixed_price',
      amount: 100,
    });
  ...
  it('rejects invalid benefit structure', async () => {
    const promoCode = createPromoCode({
      type: 'percent_discount',
      percent: 101,
    });
  ...
  it('rejects selected plan when promo is not applicable', async () => {
    const plan = createPlan({ monthlyCharge: 1000 });
    const promoCode = createPromoCode({
      type: 'amount_discount',
      amount: 100,
  ...
  it('maps duplicate usage creation to limit exceeded error', async () => {
    const promoCode = createPromoCode({
      type: 'fixed_price',
      amount: 100,
    });
